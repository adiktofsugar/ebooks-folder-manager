...
class Transaction:
    ...
    def perform(self) -> TransactionResult:
        ...
        try:
            ...
            self.log_debug(f"Processing {self.original_filepath} in {temp_dirpath}")
            for ActionKlass in ALL_ACTIONS:
                ...
                self.log_debug(f"Performing action {action.id} on {filepath}")
            ...
            if cover_hash:
                ...
                if cover_src.exists() and not cover_dst.exists():
                    ...
                    self.log_debug(f"Copied cover image to {cover_dst}")
