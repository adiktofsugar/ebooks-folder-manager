        ...
        for book_part in self.book_parts:
                ...
                if "style" in e.attrib:
                    ...
                    if "-kfx-amzn-condition" in style:
                        ...
                        if purl.path == book_part.filename and id:
                            ...
                            log.error(
                                "-kfx-amzn-condition anchor %s in file %s links to %s"
                                % (anchor, book_part.filename, url)
                            )
            ...
            if EMIT_PAGE_TEMPLATES:
                ...
                for id, cond_elem, oper, anchor in range_end_ids:
                    ...
                    if oper == "range-id.lt" and not visible_elements_before(
                        e, body[end_idx]
                    ):
                        end_idx -= 1
                        ...
                        log.error(
                            "Conditional range %s id %s has %d characters following within a top level element in %s"
                ...
                for id in anchor_ids:
                    ...
                    for root in master_roots:
                        ...
                        log.error(
                            "Conditional anchor id %s not within a master range in %s"
                            % (id, book_part.filename)
                        )
            ...
            for template_elem in body.findall("div"):
                if "style" in template_elem.attrib:
                    ...
                    if amzn_condition:
                        ...
                        if (
                            template_style.get("height", "") == "100%"
                            and template_style.get("width", "") == "100%"
                        ):
                        ...
                        if extra_style:
                            log.error(
                                "Conditional file=%s cond=%s has extra style: %s"
                                % (
                                    book_part.filename,
                                    amzn_condition,
                                    extra_style.tostring(),
                                )
                            )
                        ...
                        for i, template_child in enumerate(template_children):
                            idx = i + 1
                            context = "file=%s cond=%s idx=%d/%d" % (
                                book_part.filename,
                                amzn_condition,
                                idx,
                                len(template_children),
                            )

                            if template_child.tag == "div" and len(template_child) > 0:
                                if (
                                    len(template_child) != 1
                                    or template_child.text
                                    or template_child.tail
                                ):
                                    log.error(
                                        "Conditional container %s has incorrect length %d or content"
                                ...
                                page_align = template_child_style.pop(
                                    "-amzn-page-align", ""
                                )
                                if page_align:
                                ...
                                if (
                                    ...
                                    log.error(
                                        "Conditional container %s has incorrect style: %s"
                            ...
                            template_child_style = base_style.copy().update(
                                orig_cond_style, replace=True
                            )
                            template_child_style.pop("-kfx-style-name", None)

                            if template_child.tag in ["img", "video"] or (
                                template_child.tag == "div"
                                and "background-color" in template_child_style
                            ):
                                remove_child = False
                                ...
                                if position == "fixed":
                                    for pos, prop_name in [
                                        ...
                                        if pos in template_child_style:
                                            ...
                                            if val != "0":
                                                log.error(
                                                    "Conditional element %s has non-zero position style: %s"
                                                    % (
                                                        context,
                                                        orig_cond_style.tostring(),
                                                    )
                                                )
                                            ...
                                            if prop_name in template_child_style:
                                                if prop_name == "-amzn-float":
                                                    ...
                                                    log.error(
                                                        "Conditional element %s has conflicting position styles: %s"
                                                        % (
                                                            context,
                                                            orig_cond_style.tostring(),
                                                        )
                                ...
                                if (
                                    template_child_style.get("-amzn-page-align", "")
                                    == "none"
                                ):
                                    template_child_style.pop("-amzn-page-align")

                                is_float = (
                                    "float" in template_child_style
                                    or "-amzn-float" in template_child_style
                                )
                                collision = template_child_style.pop(
                                    "-kfx-collision", ""
                                )
                                epub_types = set(
                                    template_child_style.pop(
                                ...
                                if EMIT_PAGE_TEMPLATES:
                                    if (
                                        ...
                                        is_float and collision in ["", "always queue"]
                                    ):
                                        log.error(
                                            "Conditional element %s has conflicting float/collision styles: %s"
                                    ...
                                    if template_child.tag == "div" or (
                                        template_child.tag == "img"
                                        and "-amzn-shape-outside"
                                        not in template_child_style
                                    ):
                                        remove_child = True
                                ...
                                if epub_types:
                                    template_child_style["-kfx-attrib-epub-type"] = (
                                        epub_types
                                    )
                                ...
                                if remove_child:
                                    ...
                                    template_child_style = template_style.update(
                                        template_child_style
                                    )
                                    template_style = None
                                    ...
                                    if EMIT_PAGE_TEMPLATES:
                                        ...
                                        if not element_id:
                                            ...
                                            while True:
                                                ...
                                                if (
                                                    find_by_id(
                                                        body, element_id, required=False
                                                    )
                                                    is None
                                                ):
                                                    break
                                        ...
                                        target_style = template_child_style.partition(
                                            property_names={
                                                "-kfx-attrib-epub-type",
                                                "background-color",
                                                "display",
                                        ...
                                        pe_lines.append(
                                            "    @-amzn-page-element %s {" % element_id
                                        )
                                        ...
                                        if template_child_style:
                                            ...
                                            pe_lines.append(
                                                "      %s"
                                                % template_child_style.tostring()
                                            )

                                        pe_lines.append("    }")
                                    else:
                                        self.set_style(
                                            template_child, template_child_style
                                        )

                            elif (
                                idx == len(template_children)
                                and template_child.tag == "div"
                                and template_child_style.pop("-amzn-page-align", "")
                                == "none"
                                and template_child_style.pop("position", "") == "fixed"
                                and template_child_style.pop("-kfx-collision", "")
                                in ["", "always queue", "queue"]
                                and len(template_child_style) == 0
                            ):
                                ...
                                log.error(
                                    "Unexpected template_child %s tag=%s style=%s"
                                    % (
                                        context,
                                        template_child.tag,
                                        orig_cond_style.tostring(),
                        ...
                        if len(template_elem) == 0:
                            ...
                            if (
                                len(template_elem) > 0
                                or len(template_elem.attrib) > 0
                                or template_elem.text
                                or EMIT_EMPTY_CONDITIONS
                            ):
                                target.insert(0, template_elem)
                            ...
                            css_lines.append(
                                "  @-amzn-condition(%s) {" % amzn_condition
                            )
                            ...
                            while target.tag not in [
                                ...
                                if target.tag == "body":
                                    raise Exception(
                                        "Missing block element parent of %s in %s"
                                        % (target_id, book_part.filename)
                                    )
                            ...
                            if (
                                ...
                                template_elem.set(
                                    "style",
                                    "-kfx-media-query: not amzn-mobi; display: none",
                                )
