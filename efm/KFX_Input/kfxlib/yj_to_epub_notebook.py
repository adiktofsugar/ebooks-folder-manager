...
class KFX_EPUB_Notebook(object):
    ...
    def process_scribe_notebook_page_section(
        self, section, page_template, section_name, seq
    ):
        nmdl_canvas_width = section.pop("nmdl.canvas_width")
        nmdl_canvas_height = section.pop("nmdl.canvas_height")

        if (
            (nmdl_canvas_width, nmdl_canvas_height) in [(15624, 20832), (13726, 7350)]
            or nmdl_canvas_width in [3906, 13734]
            or nmdl_canvas_width in [3066, 6132, 12264]
            or nmdl_canvas_height > 15000
        ):
            pass
        else:
            log.warning(
                "Unexpected nmdl.canvas width=%d height=%d"
        ...
        book_part = self.new_book_part(
            filename=self.SECTION_TEXT_FILEPATH % section_name
        )
        book_part.is_fxl = True

        book_part.nmdl_template_id = section.pop(
            "nmdl.template_id", self.nmdl_template_id
        )
        if (
            book_part.nmdl_template_id is not None
            and book_part.nmdl_template_id != "$349"
            and (
                len(self.reading_orders) != 2
                or self.reading_orders[1].get("$178", "") != "note_template_collection"
                or book_part.nmdl_template_id not in self.reading_orders[1]["$170"]
            )
        ):
            log.error(
                "note_template_collection reading order does not contain nmdl.template_id %s used in section %s"
        ...
        add_meta_name_content(
            book_part.head(),
            "viewport",
            "width=%d, height=%d" % (nmdl_canvas_width, nmdl_canvas_height),
        )

        body = book_part.body()

        page_svg_elem = etree.Element(
            SVG,
        ...
        if CREATE_SVG_FILES_IN_EPUB:
            ...
            html_svg_elem = etree.SubElement(
                body,
                SVG,
                nsmap=SVG_NAMESPACES,
                attrib={
            ...
            etree.SubElement(
                ...
                attrib={
                    ...
                    "width": "100%",
                    "height": "100%",
                    XLINK_HREF: urlrelpath(
                        page_svg_filename, ref_from=book_part.filename
                    ),
                },
            )
        else:
        ...
        if "nmdl.inline_placement_type" in section:
            ...
            if inline_placement_type not in ["$670", "$669"]:
                log.error(
                    "Unexpected nmdl.inline_placement_type: %s" % inline_placement_type
                )
    ...
    def process_scribe_notebook_template_section(
        self, section, page_template, section_name
    ):
        ...
        self.process_content(
            page_template, top_level_elem, book_part, self.writing_mode, is_section=True
        )
        self.check_empty(page_template, "Section %s page_template" % section_name)
                    ...
                    if page_book_part.nmdl_template_id == section_name:
                        ...
                        if html_svg_elem is not None:
                            if len(html_svg_elem) == 3:
                                log.error(
                                    "SVG image already has template in Scribe notebook page: %s"
                                    % page_book_part.filename
                                )
                                html_svg_elem.remove(html_svg_elem[1])

                            html_svg_elem.insert(
                                1,
                                etree.Element(
                                    "image",
                                    attrib={
                            ...
                            log.error(
                                "Failed to locate the SVG image within Scribe notebook page: %s"
                                % page_book_part.filename
                            )
            else:
                log.error(
                    "Failed to locate the SVG image within Scribe notebook template: %s"
                    % book_part.filename
                )
    ...
    def process_notebook_content(self, content, parent):
        ...
        if data_type is IonSymbol:
            self.process_notebook_content(
                self.get_fragment(ftype="$608", fid=content), parent
            )
            return
    ...
    def scribe_notebook_stroke(self, content, parent, location_id):
        ...
        if nmdl_type == "nmdl.stroke_group":
            ...
            if nmdl_chunked is not True:
                log.error(
                    "%s has unexpected nmdl.chunked: %s"
                    % (self.content_context, nmdl_chunked)
                )

            if nmdl_chunk_threshold != 50:
                log.error(
                    "%s has unexpected nmdl.chunk_threshold: %s"
                    % (self.content_context, nmdl_chunk_threshold)
                )
            ...
            for name in [
                ...
                if name in nmdl_stroke_points:
                    nmdl_stroke_values[name] = decode_stroke_values(
                        nmdl_stroke_points.pop(name), nmdl_num_points, name
                    )
                else:
                    nmdl_stroke_values[name] = []

            self.check_empty(
                nmdl_stroke_points, "%s nmdl_stroke_points" % self.content_context
            )
            ...
            for thickness_index, thickness_choice in enumerate(
                THICKNESS_CHOICES.get(brush_name, [])
            ):
                thickness_diff = (
                    abs(thickness_choice - nmdl_thickness) / thickness_choice
                )
            ...
            for i in range(nmdl_num_points):
                ...
                taf = (
                    nmdl_stroke_values["nmdl.thickness_adjust_factor"][i]
                    if variable_thickness
                    else 100
                )
                daf = (
                    nmdl_stroke_values["nmdl.density_adjust_factor"][i]
                    if variable_density
                    else 100
                )

                if (
                    x < nmdl_stroke_bounds[0]
                    or x > nmdl_stroke_bounds[2]
                    or y < nmdl_stroke_bounds[1]
                    or y > nmdl_stroke_bounds[3]
                ):
                    log.error(
                        "point %d position out of range: (%d, %d) with bounds %s"
                        % (i, x, y, nmdl_stroke_bounds)
                    )

                if taf < MIN_TAF or taf > MAX_TAF:
                    log.error(
                        "point %d thickness_adjust_factor out of range: %d" % (i, taf)
                    )

                if daf < MIN_DAF or daf > MAX_DAF:
                    log.error(
                        "point %d density_adjust_factor out of range: %d" % (i, daf)
                    )
            ...
            if opacity < 1.0:
                ...
                for opacity_group_elem in svg_elem:
                    if (
                        opacity_group_elem.tag == "g"
                        and opacity_group_elem.get("opacity", "") == opacity_str
                    ):
                        break
                else:
            ...
            if "$98" in content:
                group_elem.set(
                    "transform", self.process_transform(content.pop("$98"), True)
                )
            ...
            if False:
                ...
                for x, y, t, d in points:
                    ...
                    if last_x is not None:
                        add_points_if_needed(
                            pts, last_x, last_y, last_r, last_d, x0, y0, r0, d
                        )
                ...
                for x, y, r, d in pts:
                    adjusted_d = 1.0 - (
                        (1.0 - min(max(d, 0.0), 1.0)) ** PNG_DENSITY_GAMMA
                    )
                    ...
                    for xx in range(x - int_radius, x + int_radius + 1):
                        for yy in range(y - int_radius, y + int_radius + 1):
                            if (
                                ...
                                elif rel_distance <= prng.random() * (
                                    2.0 - PNG_EDGE_FEATHERING
                                ):
                                    reduced_d = adjusted_d * (
                                        (2.0 - PNG_EDGE_FEATHERING) - rel_distance
                                    )
                ...
                with disable_debug_log():
                    png = Image.new(
                        "P" if nmdl_color != 0 else "1", (png_width, png_height)
                    )
                ...
                etree.SubElement(
                    ...
                    "image",
                    attrib={
                        "x": "%d" % nmdl_stroke_bounds[0],
                        "y": "%d" % nmdl_stroke_bounds[1],
                        "width": "%d" % bound_width,
                        "height": "%d" % bound_height,
                        XLINK_HREF: "data:image/png;base64,%s"
            ...
            log.error(
                "%s has unknown nmdl.type: %s" % (self.content_context, nmdl_type)
            )
...
def decode_stroke_values(data, num_points, name):
    ...
    if len(serial) * 2 < num_vals:
        log.error(
            "%s not enough data (%d bytes) to extract %d values"
            % (name, len(serial), num_vals)
        )
        return
    ...
    for i in range(num_vals):
        ...
        if instr & 4:
            ...
            if n == 0:
                ...
                log.error(
                    "%s pos %d instr %d - check number of bytes" % (name, i, instr)
                )
                error = True
                increment = serial.unpack("B")
        ...
        if instr & 8:
            if increment == 0:
                log.error(
                    "%s pos %d instr %d - negative zero increment" % (name, i, instr)
                )
                error = True
