...
def extract_pdf_cover(filepath: Path) -> CoverImage | None:
    ...
    try:
        ...
        if image_list:
            ...
            if largest_img:
                doc.close()
                return CoverImage.from_bytes(largest_img["image"])
...
def extract_cover(filepath: Path) -> CoverImage | None:
    ...
    if format == "epub":
        ...
        cover = extract_pdf_cover(filepath)
        if cover:
            return cover
    else:
        # Try generic extraction with PyMuPDF for other formats
        try:
            doc = pymupdf.open(filepath)
            if doc.page_count > 0:
                page = doc[0]
...
def generate_cover(
    book_hash: str, title: str | None = None, author: str | None = None
) -> CoverImage:
    ...
    if pattern_type == 0:
        ...
        for i in range(20):
            ...
            overlay_draw.polygon([(x1, y1), (x2, y2), (x3, y3)], fill=(*color, alpha))
            ...
            for row, pattern in enumerate(cloud_pattern):
                for col, char in enumerate(pattern):
                    if char == "#":
                        ...
                        draw.rectangle(
                            [(x, y), (x + block_size, y + block_size)], fill=cloud_white
                        )
            ...
            for row, pattern in enumerate(hill_pattern):
                for col, char in enumerate(pattern):
                    if char == "#":
                        ...
                        color = (
                            hill_green if (row + col + i) % 3 != 0 else hill_dark_green
                        )
                        draw.rectangle(
        ...
        for i in range(num_pipes):
            ...
            draw.rectangle(
                [
                    (
                        pipe_x + 2 * block_size - block_size // 2,
                        pipe_y + 2 * block_size,
                    ),
                    (pipe_x + 2 * block_size, pipe_y + pipe_height),
                ],
        ...
        for x in range(0, width, block_size):
            for y in range(height - ground_height, height, block_size):
                ...
                if (x // block_size + y // block_size) % 2 == 0:
                    ...
                    draw.rectangle(
                        [(x, y), (x + block_size, y + block_size)], fill=ground_dark
                    )
        ...
        for i in range(num_blocks):
            ...
            draw.rectangle(
                ...
                fill=block_yellow,
                outline=block_orange,
                width=2,
            )

            # Draw question mark
            draw.text(
                (block_x + block_size - 5, block_y + 2),
                "?",
                fill=block_orange,
        ...
        creature_patterns = [
            # Mushroom (like Mario mushroom)
            {
                ...
                "colors": [
                    ...
                    (255, 255, 255),
                    (200, 76, 12),
                ],  # red cap, white spots, brown stem
            },
            # Bird
            {
                "pattern": [
            ...
            # Flower
            {
                ...
                "colors": [
                    ...
                    (255, 255, 255),
                    (0, 128, 0),
                ],  # pink petals, white center, green stem
            },
            # Bunny
            {
                "pattern": [
        ...
        for i in range(num_creatures):
            ...
            while attempts < 10:
                ...
                for pos in used_positions:
                    if (
                        abs(creature_x - pos[0]) < 150
                        and abs(creature_y - pos[1]) < 150
                    ):
                        overlap = True
                        break
    ...
    if title or author:
        ...
        text_draw.rectangle(
            [(0, height - 200), (width, height)], fill=(255, 255, 255, 200)
        )
        ...
        if author:
            text_draw.text(
                (20, y_offset + 10), f"by {author}", fill=(64, 64, 64), font=author_font
            )
    ...
    return CoverImage.from_bytes(output.getvalue())
...
def embed_pdf_cover(filepath: Path, cover: CoverImage) -> None:
    ...
    doc.close()
    logger.info(f"Inserted cover image as first page of PDF: {filepath}")
...
def embed_cover(filepath: Path, cover: CoverImage) -> None:
    ...
    if format == "pdf":
        ...
        embed_epub_cover(filepath, cover)
    else:
        # Format not supported for cover embedding
        raise ValueError(f"Cannot set cover for format: {format}")
