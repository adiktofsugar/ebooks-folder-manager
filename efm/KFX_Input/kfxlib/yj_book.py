...
class YJ_Book(BookStructure, BookPosLoc, BookMetadata, KpfBook):
    ...
    def convert_to_epub(self, epub2_desired=False, force_cover=False, progress_fn=None):
        ...
        result = KFX_EPUB(
            ...
            progress=make_progress(progress_fn),
        ).decompile_to_epub()
        self.final_actions()
        return result
    ...
    def convert_to_cbz(self, split_landscape_comic_images=False, progress_fn=None):
        ...
        result = KFX_IMAGE_BOOK(self).convert_book_to_cbz(
            split_landscape_comic_images, make_progress(progress_fn)
        )
        self.final_actions()
        return result
    ...
    def convert_to_kpf(
        ...
        try:
            result = conversion_sequence.convert_to_kpf(
        ...
        if self.datafile.is_real_file and os.path.isdir(self.datafile.name):
            ...
            with self.datafile.as_ZipFile() as zf:
                for info in zf.infolist():
                    if posixpath.basename(info.filename).lower() in [
                        "book.ion",
                        "book.kdf",
                    ]:
                        self.container_datafiles.append(self.datafile)
            ...
            raise Exception(
                "Unknown main file type %s. Must be azw8, ion, kfx, kfx-zip, kpf, or zip."
                % self.datafile.ext
            )
    ...
    def check_located_file(self, name, data=None, parent=None):
        ...
        if basename.startswith("._") or basename == "BookManifest.kfx":
            ...
            ext in [".azw", ".azw8", ".azw9", ".kfx", ".md", ".res", ".yj"]
            or basename == "nbk"
        ):
            ...
            if datafile.name.endswith("metadata.kfx"):
                ...
                if expanded_data:
                    return self.get_container(
                        DataFile(datafile.name + ".decompressed", expanded_data)
                    )
    ...
    def expand_compressed_container(self, data):
        ...
        return (
            expanded_data
            if expanded_data and expanded_data.startswith(KfxContainer.SIGNATURE)
            else None
        )
