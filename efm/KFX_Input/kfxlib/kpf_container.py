...
class KpfContainer(YJContainer):
    ...
    def deserialize(self, ignore_drm=False):
        ...
        ACTION_FRAGMENTS_SCHEMA = (
            "CREATE TABLE local_action_fragments(action_id char(40), payload_type char(10), payload_value blob, "
        ...
        if ACTION_FRAGMENTS_SCHEMA in schema:
            ...
            for action_id, payload_type, payload_value in cursor.execute(
                "SELECT * FROM local_action_fragments;"
            ):
                log.warning(
                    "Action: action_id=%s, payload_type=%s, payload_value=%s"
                    % (action_id, payload_type, payload_value)
                )

        DELTA_FRAGMENTS_SCHEMA = (
            "CREATE TABLE local_delta_fragments(id char(40), payload_type char(10), payload_value blob, "
        ...
        if FRAGMENTS_SCHEMA in schema:
            ...
            for condition in [
                " WHERE id = '$ion_symbol_table'",
                " WHERE id = 'max_id'",
                "",
            ]:
                delta_fragments = {}
                if has_delta_fragments:
                    for id, payload_type, payload_value, deleted in cursor.execute(
                        "SELECT * FROM local_delta_fragments%s;" % condition
                    ):
                ...
                for id, payload_type, payload_value in cursor.execute(
                    "SELECT * FROM fragments%s;" % condition
                ):
            ...
            if payload_data is None:
                ...
                if not self.have_symbol_table_or_max_id:
                    ...
                    self.symtab.import_shared_symbol_table(
                        "YJ_symbols", max_id=max_id - len(SYSTEM_SYMBOL_TABLE.symbols)
                    )
                ...
                if element_type == "auxiliary_data":
                    ...
                    self.fragments.append(
                        YJFragment(
                            ftype=ftype,
                            fid=fid,
                            value=IonStruct({IS("$598"): fid, IS("$258"): IonList()}),
                        )
                    )
                ...
                if not isinstance(value, IonAnnotation):
                    ...
                    log.error(
                        "KDF fragment should have one annotation: %s" % repr(value)
                    )
                ...
                self.fragments.append(
                    YJFragment(
                        ftype=ftype, fid=fid, value=self.deref_kfx_ids(value.value)
                    )
                )
