...
from efm.cover import generate_cover
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate random book covers using EFM function", add_help=False
    )
    parser.add_argument(
        "-n", "--number", type=int, default=20, help="Number of covers to generate"
    )
    parser.add_argument(
        "-o", "--output", type=str, default="generated_covers", help="Output directory"
    )
    ...
    for i in range(args.number):
        ...
        cover_image = generate_cover(book_hash, title, author)

        # Save cover
        filename = f"cover_{i + 1:03d}.png"
        cover_image.save(output_dir / filename)
