...
class KFX_IMAGE_BOOK(object):
    ...
    def convert_book_to_cbz(self, split_landscape_comic_images, progress):
        ...
        if yj_metadata.authors:
            comic_book_info["credits"] = [
                {"person": author, "role": "Writer"} for author in yj_metadata.authors
            ]
        ...
        cbz_metadata = (
            {"ComicBookInfo/1.0": comic_book_info} if comic_book_info else None
        )
    ...
    def convert_book_to_pdf(self, split_landscape_comic_images, progress):
            ...
            for toc_entry in toc:
                ...
                if eid is not None:
                    ...
                    if toc_pid is not None:
                        ...
                        for ordered_page_num, ordered_page_pid in enumerate(
                            ordered_image_pids
                        ):
                        ...
                        log.error(
                            "Failed to locate pid toc_entry: label=%s, eid=%s, eid_offset=%s"
        ...
        return combine_images_into_pdf(
            ordered_images, pdf_metadata, is_rtl, kfx_epub.ncx_toc
        )
    ...
    def get_ordered_images(
        ...
        ordered_image_resources, ordered_image_resource_pids, content_pos_info = (
            self.book.get_ordered_image_resources()
        )
        ...
        ordered_image_pids = []
        split_image_count = 0
        for cnt, (fid, pid) in enumerate(
            zip(ordered_image_resources, ordered_image_resource_pids)
        ):
            image_resource = self.get_resource_image(fid)
            if image_resource is not None:
                if (
                    split_landscape_comic_images
                    and image_resource is not None
                    and is_comic
                    and image_resource.format != "$565"
                    and image_resource.width > image_resource.height
                ):
                    split_image_count += 1

                    new_width = image_resource.width // 2
                    left_image = crop_image(
                        image_resource.raw_media,
                        image_resource.location,
                    ...
                    left = ImageResource(
                        ...
                        suffix_location(image_resource.location, "-L"),
                        left_image,
                        image_resource.height,
                        new_width,
                    )
                    ...
                    right = ImageResource(
                        image_resource.format,
                        suffix_location(image_resource.location, "-R"),
                        right_image,
                        image_resource.height,
                        new_width,
                    )
        ...
        if split_image_count:
            log.warning(
                "Split %d landscape comic images into left/right image pairs"
                % split_image_count
            )
        ...
        if num_pages and len(ordered_images) < num_pages:
            log.warning(
                "Expected %d pages but found only %d page images in book"
                % (num_pages, len(ordered_images))
            )
    ...
    def get_resource_image(self, resource_name, ignore_variants=False):
        ...
        if "$636" in resource:
            ...
            for row in yj_tiles:
                for tile_location in row:
                    tile_raw_media_frag = self.book.fragments.get(
                        ftype="$417", fid=tile_location
                    )
                    tiles_raw_media.append(
                        None
                        if tile_raw_media_frag is None
                        else tile_raw_media_frag.value
                    )
        ...
        return ImageResource(
            resource_format, location, raw_media, resource_height, resource_width
        )
...
def combine_images_into_pdf(ordered_images, metadata=None, is_rtl=False, outline=None):
    ...
    for image_resource in ordered_images:
        image_resource_formats[SYMBOL_FORMATS[image_resource.format].upper()].add(
            image_resource.location
        )
        ...
        if image_resource.format == "$565":
            if (
                combined_pdf_images
                and combined_pdf_images[-1].format == "$565"
                and combined_pdf_images[-1].location == image_resource.location
            ):
    ...
    if len(combined_pdf_images) == 1 and combined_pdf_images[0].entire_resource_used():
        ...
        try:
            ...
            log.error(
                "pypdf PdfWriter error in clone_from %s: %s"
                % (combined_pdf_images[0].location, repr(e))
            )
            return None
    else:
        combined = True
        ...
        for image_resource in combined_pdf_images:
            try:
                if image_resource.entire_resource_used():
                    ...
                    log.warning(
                        "Using PDF %s pages %s of %d"
                        % (
                            image_resource.location,
                            repr(image_resource.page_nums),
                            image_resource.total_pages,
                        )
                    )

                    for page_range in image_resource.page_number_ranges():
                        writer.append(
                            fileobj=io.BytesIO(image_resource.raw_media),
                            pages=page_range,
                        )
            except Exception as e:
                log.error(
                    "pypdf PdfWriter error appending %s: %s"
    ...
    if combined:
        log.info(
            "Combined %s resources into a %d page PDF file"
...
def add_pdf_outline(pdf_writer, outline_entries, parent=None):
    for outline_entry in outline_entries:
        new_entry = pdf_writer.add_outline_item(
            outline_entry.title, outline_entry.page_num, parent=parent
        )
...
def combine_images_into_cbz(ordered_images, metadata=None):
    ...
    for image_resource in ordered_images:
        ...
        if image_resource.format in {"$286", "$285", "$284"}:
            ...
            image_data, fmt = convert_jxr_to_jpeg_or_png(
                image_resource.raw_media, image_resource.location
            )
    ...
    with zipfile.ZipFile(cbz_file, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for i, image_resource in enumerate(page_images):
            zf.writestr(
                "%04d.%s" % (i + 1, SYMBOL_FORMATS[image_resource.format]),
                image_resource.raw_media,
            )
    ...
    log.info(
        "Combined %s resources into a %d page CBZ file"
        % (list_counts(image_resource_formats), len(ordered_images))
    )
