...
class BookPosLoc(object):
    ...
    def collect_content_position_info(
        self,
        keep_footnote_refs=True,
        skip_non_rendered_content=False,
        ...
        def collect_section_position_info(section_name):
            ...
            def extract_position_data(
                data, current_eid, content_key, list_index, list_max, advance, note_refs
            ):
                def have_content(
                    eid,
                    length,
                    ...
                    if note_refs and (text or image_resource):
                        ...
                        for start_, len_ in note_refs_copy:
                            if start_ >= text_len:
                                ...
                                if text:
                                    ...
                                    if (
                                        sum([1 if c.isalpha() else 0 for c in remove])
                                        < 3
                                    ):
                                        text = (
                                            text[:start_]
                        ...
                        if text and len(text) != text_len:
                            log.error(
                                "text len changed from %d to %d" % (text_len, len(text))
                            )
                    ...
                    if last_chunk is not None and last_chunk.eid == eid:
                        if last_chunk.image_resource is None or image_resource is None:
                            ...
                            log.error(
                                "Image resources %s and %s together in eid %d"
                                % (last_chunk.image_resource, image_resource, eid)
                            )
                    ...
                    if self.cpi_processing_story_ and eid_cond_info:
                        if not self.cpi_fixed_:
                            if DEBUG_KIM_TEMPLATES:
                                log.info(
                                    "Starting story template processing for section %s..."
                                    % section_name
                                )
                            ...
                            for idx, ct in enumerate(eid_cond_info):
                                if ct.oper == "$348":
                                    ...
                                    if (
                                        prev_equal_ct_idx == idx - 1
                                        and ct.start_eid == prev_equal_ct_start_eid
                                        and ct.start_eid_offset
                                        == prev_equal_ct_start_eid_offset
                                    ):
                                        if DEBUG_KIM_TEMPLATES:
                                            log.info("moving ct - duplicate: %s" % ct)

                                        ct.start_eid = ct.end_eid = story_start_eid
                                        ct.start_eid_offset = ct.end_eid_offset = (
                                            story_start_eid_offset
                                        )
                                        moves.append(idx)
                                    else:
                                    ...
                                    while (prev_eid, prev_eid_offset) in equals:
                                        ...
                                        if DEBUG_KIM_TEMPLATES:
                                            log.info(
                                                "updated prev offset: %s+%d"
                                                % (prev_eid, prev_eid_offset)
                                            )
                                    ...
                                    if ct.start_eid == ct.end_eid and (
                                        ...
                                        or (
                                            ct.oper == "$298"
                                            and ct.start_eid_offset == ct.end_eid_offset
                                        )
                                    ):
                                        if DEBUG_KIM_TEMPLATES:
                                            log.info(
                                                "moving ct - ends before start: %s" % ct
                                            )
                        ...
                        while eid_cond_info:
                            ...
                            for idx_, ct_ in enumerate(eid_cond_info):
                                ...
                                if ct_.start_eid == eid and (
                                    ct is None
                                    or ct_.start_eid_offset < ct.start_eid_offset
                                ):
                                    idx = idx_
                                    ct = ct_
                            ...
                            if ct.start_eid_offset < eid_offset:
                                log.error(
                                    "conditional %s is before %s+%d-%d"
                                    % (ct, eid, eid_offset, eid_offset + length)
                                )
                                ...
                                if DEBUG_KIM_TEMPLATES:
                                    log.info(
                                        "inserting conditional %s before %s+%d-%d"
                                        % (ct, eid, eid_offset, eid_offset + length)
                                    )
                                ...
                                if DEBUG_KIM_TEMPLATES:
                                    log.info(
                                        "conditional %s falls inside %s+%d-%d"
                                        % (ct, eid, eid_offset, eid_offset + length)
                                    )

                                split_len = ct.start_eid_offset - eid_offset
                                split_text = (
                                    text[:split_len] if text is not None else None
                                )
                                section_pos_info.append(
                                    ContentChunk(
                                        self.cpi_pid_,
                                ...
                                if DEBUG_KIM_TEMPLATES:
                                    log.info(
                                        "conditional %s falls past %s+%d-%d"
                                        % (ct, eid, eid_offset, eid_offset + length)
                                    )
                ...
                if data_type is IonAnnotation:
                    ...
                    for fc in data:
                        extract_position_data(
                            fc, current_eid, content_key, None, None, advance, note_refs
                        )
                    ...
                    if skip_non_rendered_content and (
                        data.get("$69", False) or typ == "$439"
                    ):
                        return
                            ...
                            for pt in data["$141"]:
                                ...
                                if "$171" in pt:
                                    ...
                                    if (
                                        ...
                                        and condition[0] in ["$294", "$299", "$298"]
                                    ):
                                        condition_eid_offset = self.anchor_eid_offset(
                                            condition[2][1]
                                        )
                                        if condition_eid_offset is not None:
                                            eid_cond_info.append(
                                                ConditionalTemplate(
                                                    condition_eid_offset[0],
                                                    condition_eid_offset[1],
                                                    condition[0],
                                                    section_pos_info[:],
                                                )
                                            )
                                        ...
                                        log.error(
                                            "KIM condition is not in expected format: %s"
                                            % repr(condition)
                                        )
                                    ...
                                    if have_range:
                                        ...
                                        eid_cond_info.append(
                                            ConditionalTemplate(
                                                None, 0, "$298", final_range_pos_info
                                            )
                                        )

                                    eid_cond_info.append(
                                        ConditionalTemplate(
                                            None, 0, "$348", non_conditional_pos_info
                                        )
                                    )
                    ...
                    if (not keep_footnote_refs) and "$142" in data:
                        ...
                        for style_event in data["$142"]:
                            if style_event.get("$616") == "$617":
                                note_refs.append(
                                    (style_event["$143"], style_event["$144"])
                                )
                    ...
                    if "$146" in data:
                        if typ == "$274":
                            ...
                            extract_position_data(
                                data["$146"], None, "$146", None, None, False, note_refs
                            )
                        else:
                            extract_position_data(
                                data["$146"],
                    ...
                    for fk, fv in data.items():
                        if ion_type(fv) != IonString and fk not in {
                            ...
                            extract_position_data(
                                fv, current_eid, fk, None, None, advance, note_refs
                            )
                    ...
                    if include_background_images:
                        if "$479" in data:
                            have_content(
                                current_eid, 0, False, image_resource=data.get("$479")
                            )

                        if "$157" in data:
                            style_value = self.fragments[
                                YJFragmentKey(ftype="$157", fid=data["$157"])
                            ].value
                            if "$479" in style_value:
                                have_content(
                                    ...
                                    0,
                                    False,
                                    image_resource=style_value.get("$479"),
                                )
                ...
                if new_note_refs:
                    ...
                    log.info(
                        "end extract_position_data eid=%s key=%s idx=%s/%s adv=%s note_refs=%s: %s"
                        % (
                            current_eid,
                            content_key,
                            list_index,
        ...
        for section_name, stories in section_stories.items():
            if not (
                len(stories) == 1
                or (self.is_print_replica and len(stories) == 2)
                or (self.is_pdf_backed_fixed_layout and len(stories) in [2, 3])
                or (self.is_scribe_notebook and len(stories) > 0)
            ):
                log.error(
            ...
            for i, pi in enumerate(contains):
                ...
                if i > 0:
                    ...
                    if eid_offset != pid - eid_start_pos[eid]:
                        if (
                            self.has_illustrated_layout_conditional_page_template
                            or has_mathml
        ...
        while not (map.at_end() and content.at_end()):
            ...
            if map.chunk().__eq__(content.chunk(), compare_pids=compare_pids):
                if REPORT_POSITION_DATA:
                    log.info(
                        "position_id at %s %s%s"
                        % (
                            "cidx=%d midx=%d" % (content.index, map.index)
                            if content.index != map.index
                            else "idx=%d" % map.index,
                            "mpid=%d " % map.chunk().pid
    ...
    def collect_location_map_info(self, pos_info):
        ...
        if not (self.is_dictionary or self.is_scribe_notebook or self.is_kpf_prepub):
            ...
            if has_yj_location_pid_map and (
                ...
                log.error(
                    "yj.location_pid_map with FC kfxgen.positionMaps=%s textbook=%s"
